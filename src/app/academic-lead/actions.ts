...
export async function publishPracticeSetAction(_: ContentActionState, formData: FormData): Promise<ContentActionState> { try { const questionIds = String(formData.get("questionIds") ?? "").split(/[\s,]+/).filter(Boolean); const mediaByQuestion = String(formData.get("mediaReferences") ?? "").split("\n").map((line) => line.trim()).filter(Boolean).map((line) => { const [questionId, ...mediaIds] = line.split("|"); return { questionId: questionId!.trim(), mediaIds: mediaIds.join("|").split(",").map((id) => id.trim()).filter(Boolean) }; }); const id = await publishPracticeSet(await actorFor(), { questionIds, mediaByQuestion }); revalidatePath("/academic-lead"); return { success: `Practice set published: ${id}` }; } catch (error) { return failure(error, "PRACTICE_SET_PUBLISH_FAILED"); } }
