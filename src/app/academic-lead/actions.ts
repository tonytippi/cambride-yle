...
import { acceptException, approveContent, approvePracticeSet, createManualMedia, createManualQuestion, createPracticeSetDraft, publishContent, publishPracticeSet, recordPhonePreview, rejectContent, requestAiDraft, retireContent, retirePracticeSet, reviseMedia, reviseQuestion, submitForReview, submitPracticeSetForReview, validateContent } from "@/features/content/application/content";
...
const contentBase = (formData: FormData) => ({ paper: String(formData.get("paper")) as "listening", part: Number(formData.get("part")), engine: String(formData.get("engine")) as "picture_true_false", primaryTargetId: String(formData.get("primaryTargetId")), supportingTargetIds: String(formData.get("supportingTargetIds") ?? "").split(",").map((id) => id.trim()).filter(Boolean), topicIds: [String(formData.get("topicId"))], guidanceId: String(formData.get("guidanceId")), evidenceDimensions: { spelling: values(formData, "spelling"), colours: values(formData, "colours"), positions: values(formData, "positions") }, estimatedDurationSeconds: Number(formData.get("estimatedDurationSeconds")), accessibilityMetadata: { altText: String(formData.get("altText")), ...(String(formData.get("choiceLabel") ?? "").trim() ? { choiceLabel: String(formData.get("choiceLabel")).trim() } : {}) }, provenance: { source: String(formData.get("source")), rightsReference: String(formData.get("rightsReference")) } });
const values = (formData: FormData, name: string) => String(formData.get(name) ?? "").split(",").map((value) => value.trim()).filter(Boolean);
...
export async function createPracticeSetDraftAction(_: ContentActionState, formData: FormData): Promise<ContentActionState> { try { const questionIds = String(formData.get("questionIds") ?? "").split(/[\s,]+/).filter(Boolean); const id = await createPracticeSetDraft(await actorFor(), { title: String(formData.get("title") ?? ""), questionIds }); revalidatePath("/academic-lead"); return { success: `Practice set draft created: ${id}` }; } catch (error) { return failure(error, "PRACTICE_SET_DRAFT_FAILED"); } }
// eslint-disable-next-line no-unused-vars
const practiceSetWorkflow = (useCase: (_actor: Awaited<ReturnType<typeof actorFor>>, _input: { practiceSetId: string }) => Promise<unknown>, success: string) => async (_: ContentActionState, formData: FormData): Promise<ContentActionState> => { try { await useCase(await actorFor(), { practiceSetId: String(formData.get("practiceSetId")) }); revalidatePath("/academic-lead"); return { success }; } catch (error) { return failure(error, "PRACTICE_SET_WORKFLOW_FAILED"); } };
export const submitPracticeSetForReviewAction = practiceSetWorkflow(submitPracticeSetForReview, "Practice set submitted for review.");
export const approvePracticeSetAction = practiceSetWorkflow(approvePracticeSet, "Practice set approved.");
export const publishPracticeSetAction = practiceSetWorkflow(publishPracticeSet, "Practice set published.");
