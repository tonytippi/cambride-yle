...
export async function publishPracticeSetAction(_: ContentActionState, formData: FormData): Promise<ContentActionState> { try { const questionIds = String(formData.get("questionIds") ?? "").split(/[\s,]+/).filter(Boolean); const id = await publishPracticeSet(await actorFor(), { questionIds }); revalidatePath("/academic-lead"); return { success: `Practice set published: ${id}` }; } catch (error) { return failure(error, "PRACTICE_SET_PUBLISH_FAILED"); } }
