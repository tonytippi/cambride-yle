...
export function Preparation({ initial }: { initial: PracticeResult<PracticePreparation> }) {
  ...
  return <main className="shell"><section className="preparation"><p className="eyebrow">Practice preparation</p><h1>{data?.title ?? "Practice activity"}</h1>{"error" in preparation ? <><p className="error" role="alert">{preparation.error.message}</p><div className="preparation-actions"><button onClick={retry}>Retry</button><a className="practice-action secondary-link" href="/learner">Leave</a></div></> : <>{data!.assets.length ? <><p id="preparation-status" role="status">Check that essential media is ready before you start.</p><ul className="media-readiness" aria-describedby="preparation-status">{data!.assets.map((asset) => <li key={asset.id}><strong>{asset.type === "audio" ? "Audio" : "Image"}</strong><span aria-live="polite">{states[asset.id] ?? "Preparing"}</span></li>)}</ul></> : <p id="preparation-status" role="status">No essential media is required for this practice.</p>}<div className="preparation-actions"><button disabled={!ready || starting} onClick={start}>{starting ? "Starting..." : "Start"}</button>{data!.assets.length ? <button className="secondary" onClick={() => checkAssets()}>Check again</button> : null}<a className="practice-action secondary-link" href="/learner">Leave</a></div></>}</section></main>;
}
