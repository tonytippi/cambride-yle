...
import { createSessionCleanupToken } from "../infrastructure/session-cleanup";
...
export async function requireRole(roles: Role[]): Promise<Actor> { const token = (await cookies()).get(sessionCookieName)?.value ?? ""; const actor = await getActorBySessionToken(token); if (!actor) redirect(token ? `/api/auth/session-expired?token=${encodeURIComponent(createSessionCleanupToken(token))}` : "/sign-in"); if (!roles.includes(actor.role)) redirect(`${roleHome(actor.role)}?notice=${encodeURIComponent("You do not have access to that page.")}`); return actor; }
