...
export const practiceAttempts = pgTable("practice_attempts", {
  ...
  check("practice_attempts_lifecycle_check", sql`((${table.status} = 'open' AND ${table.submittedAt} IS NULL) OR (${table.status} = 'submitted' AND ${table.submittedAt} IS NOT NULL)) AND ${table.lastSavedAt} >= ${table.createdAt} AND (${table.submittedAt} IS NULL OR ${table.submittedAt} >= ${table.createdAt})`),
