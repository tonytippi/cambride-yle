...
test.beforeAll(async () => {
  ...
  try {
    ...
    await sql.begin(async (transaction) => {
      ...
      await transaction`INSERT INTO practice_attempts (id, learner_id, practice_set_id, practice_set_version_id, status, submitted_at, last_saved_at, created_at, finalisation_key, submitted_title, submitted_presentation, expected_review_item_count, review_snapshot_items, final_timing, playback_snapshot) VALUES (${fixture.attemptId}, ${fixture.learnerId}, ${fixture.setId}, ${fixture.setId}, 'submitted', ${submittedAt}, ${submittedAt}, ${submittedAt}, ${crypto.randomUUID()}, 'Animal listening', '{"paper":"listening","part":"1"}'::jsonb, 1, ${JSON.stringify([{ id: itemId, position: 1 }])}::jsonb, '{}'::jsonb, '[]'::jsonb)`;
      await transaction`INSERT INTO practice_attempt_review_items (id, attempt_id, practice_set_item_id, position, outcome, evidence_label, approved_answer, approved_answer_label, presentation, answer_policy_version, curriculum_tags) VALUES (${fixture.reviewItemId}, ${fixture.attemptId}, ${itemId}, 1, 'incorrect', 'needs_practice', 'true'::jsonb, 'True', '{}'::jsonb, 'fixture', ${transaction.json({ evidenceTargets: [{ id: fixture.targetId, label: "animals" }] })})`;
      await transaction`INSERT INTO submitted_evidence_facts (id, attempt_id, review_item_id, learner_id, practice_set_id, paper, part, language_target_id, language_target, automatic_outcome, submitted_at) SELECT ${crypto.randomUUID()}, ${fixture.attemptId}, ${fixture.reviewItemId}, ${fixture.learnerId}, ${fixture.setId}, 'listening', '1', ${fixture.targetId}, 'animals', 'incorrect', submitted_at FROM practice_attempts WHERE id = ${fixture.attemptId}`;
    });
  } finally {
    await sql.end();
  }
